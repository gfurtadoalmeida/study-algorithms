...
{
    public sealed class IndexedMaxPQ<T> : IEnumerable<Int32> where T : IComparable<T>
    {
        ...
        public IndexedMaxPQ(Int32 capacity)
        {
            this.Count = 0;
        ...
        public void Delete(Int32 index)
        {
            ...
            this._itens[index] = null;
            this._indexList[index] = -1;
        }

        public Int32 DeleteMax()
        {
            if (this.Count == 0)
                throw new InvalidOperationException();

            Int32 max = this._binHeap[1];

            this.Exchange(1, this.Count--);
            this.Sink(1);

            this._indexList[max] = -1;
            this._itens[max] = null;
        ...
        private Boolean IsLess(Int32 i, Int32 j)
        {
        ...
        private void Sink(Int32 index)
        {
            while (2 * index <= this.Count)
            {
                ...
                if (i < this.Count && this.IsLess(i, i + 1))
                {
                    i++;
                }

                if (!this.IsLess(index, i))
                    break;
